...
.device {
  ...
  height:163px;
  float:right;
  background-image:url('<?php echo $path; ?>/files/HomeEnergySmall.png');
  position:relative;
}
...
<div id="devices" style="text-align:left"></div>
</div>
    ...
    $.ajax({ 
        ...
        success: function(result) {
            ...
            for (var devicekey in devices) {
                out += "<div style='background-color:#44b3e2; color:#fff'>";
                ...
                out += "  <div style='padding:20px; background-color:rgba(255,255,255,0.1); font-size:16px; line-height:30px'>";
                ...
                out += "    <br><button class='btn configure' device='"+devicekey+"'>Configure</button>";
                out += "    </div>";
                ...
                if (!devices[devicekey].verified) {
                    ...
                    out += "<br><button class='btn configure' device='"+devicekey+"'>Reset and re-configure</button>";
                } else {
                    selected_configuration[devicekey] = configkey;
                    out += "<br><button class='btn changeconfiguration' device='"+devicekey+"'>Change configuration</button>";
                }
                }
                out += "    </div>";
            ...
            for (var devicekey in devices) {
                var configkey = devices[devicekey].configured;
                var configuration = devices[devicekey].configurations[configkey];
                $(".device[device="+devicekey+"]").css("background-image","url('"+path+"/files/"+configuration.image+"')");
